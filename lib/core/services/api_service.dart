...
class ApiService {
  ...
  void _saveTokensFromResponse(Map<String, dynamic> data) async {
    ...
    if (access != null && access.isNotEmpty) {
      ...
      debugPrint('Token saved from response');
    }
  }
  ...
  Future<User?> getCurrentUser() async {
    ...
    debugPrint('getCurrentUser — userId: $userId  token: $_accessToken');
  ...
  Future<void> changePassword(ChangePasswordRequest request) async {
    final response =
    await _put(ApiConstants.changePassword, request.toJson());
    debugPrint('CHANGE PW ${response.statusCode}: ${response.body}');
    if (response.statusCode != 200 && response.statusCode != 204) {
      throw Exception(response.body);
    }
  }
